...
- **Scope of this ballpark item:** **Model A only** at the executable layer (`dolo-plus-draft.yaml`, `verification.md`). Model B is documented in `bellman-excerpt.md` §10 for completeness but is intentionally not encoded as a YAML stage and is not covered by the verification claims. Encoding Model B is an out-of-scope follow-up (see "Common next tasks" below). This decision is mirrored in `index.md`, `bellman-excerpt.md` §10 banner, and an explicit `# unresolved:` block at the foot of `dolo-plus-draft.yaml`.
...
- **Model B is intentionally not encoded in the YAML.** `bellman-excerpt.md` §10 gives the full perch decomposition for Model B and an EGM-feasibility note, but `dolo-plus-draft.yaml` covers only Model A and this is the **declared scope** for this ballpark item (see Scope bullet above). A Model-B YAML would need a second control `l`, a labor-income term in the budget, and the intratemporal FOC $\tilde{c}/l = \eta \bar w e / (1-\eta)$ to close conditional inversion; that work is tracked as an out-of-scope follow-up in "Common next tasks."
